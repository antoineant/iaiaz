...
export default function MifaSettingsPage() {
  return (
...
function MifaSettingsContent() {
  const t = useTranslations("mifa.settings");
  ...
  const loadInvites = async () => {
    try {
      const res = await fetch("/api/mifa/invite");
      if (res.ok) {
  ...
  const loadData = async () => {
    try {
      const [res] = await Promise.all([
        fetch("/api/mifa/analytics?days=7"),
        loadInvites(),
      ]);
      if (!res.ok) return;
      const data = await res.json();
      ...
      const withControls = await Promise.all(
        childMembers.map(async (child: { user_id: string; display_name: string; supervision_mode: string; age_bracket: string }) => {
          const [controlRes, profileRes] = await Promise.all([
            fetch(`/api/mifa/controls?childId=${child.user_id}`),
            fetch(`/api/mifa/child-profile?childId=${child.user_id}`),
          ]);
  ...
  const resetChildPassword = async (childUserId: string) => {
    ...
    setResetMsg({});
    try {
      const res = await fetch("/api/mifa/reset-password", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const getStatusBadge = (invite: MifaInvite) => {
  ...
  const getInviteActions = (invite: MifaInvite) => {
    const isExpired = invite.status === "pending" && new Date(invite.expires_at) < new Date();

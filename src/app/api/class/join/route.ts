...
// GET /api/class/join?token=xxx OR ?code=ABC123 - Get class info by join token or code (public)
export async function GET(request: Request) {
  try {
    const { searchParams } = new URL(request.url);
    // Accept either token (32-char hex) or code (6-8 char alphanumeric)

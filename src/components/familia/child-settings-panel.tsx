...
import { ACCENT_COLORS, type ThemeColor } from "@/lib/familia/theme";
...
export function ChildSettingsPanel({
  ...
  const t = useTranslations("familia.childSettings");
  ...
  const loadAssistants = useCallback(async () => {
    try {
      const res = await fetch("/api/familia/assistants");
      if (res.ok) {
        const data = await res.json();
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch("/api/familia/my-stats");
        if (res.ok) {
          const data = await res.json();
          setStats(data);
          setStatsLoaded(true);
  ...
  const saveEdit = async () => {
    ...
    try {
      const res = await fetch(`/api/familia/assistants/${editingId}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          name: editName.trim(),
  ...
  const handleDelete = async (id: string) => {
    setDeletingId(null);
    try {
      const res = await fetch(`/api/familia/assistants/${id}`, { method: "DELETE" });
      if (res.ok) {
  ...
  const moveAssistant = async (index: number, direction: -1 | 1) => {
    ...
    try {
      await Promise.all([
        fetch(`/api/familia/assistants/${newList[index].id}`, {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ sort_order: index }),
        }),
        fetch(`/api/familia/assistants/${newList[newIndex].id}`, {
          method: "PUT",
          headers: { "Content-Type": "application/json" },

...
const PARENT_PROMPT_TEMPLATE = `You are an AI assistant on Familia by iaiaz, a family learning platform.
...
const GUARDIAN_PROMPT_TEMPLATE = `You are an AI assistant on Familia by iaiaz, a family learning platform.
...
<familia_meta>{"subject":"maths","topic":"fractions","type":"homework","struggle":false}</familia_meta>
...
export function buildFamiliaSystemPrompt(
  ageBracket: string,
  supervisionMode: string,
  userName: string,
...
/**
 * Parses and strips <familia_meta> tags from AI response content.
 * Returns the clean content (without meta tag) and parsed metadata.
 */
export function parseFamiliaMetadata(content: string): {
  cleanContent: string;
  metadata: FamiliaMetadata | null;
} {
  const metaRegex = /<familia_meta>([\s\S]*?)<\/familia_meta>/;
  ...
  try {
    const metadata = JSON.parse(match[1]) as FamiliaMetadata;
...
/**
 * For streaming: buffers content and strips familia_meta tag in real-time.
 * Returns the content safe to send to client and any detected metadata.
 */
export function createFamiliaMetaStripper() {
  let buffer = "";
  let metadataFound: FamiliaMetadata | null = null;
  const TAG_START = "<familia_meta>";
  ...
  return {
    ...
    processChunk(chunk: string): string {
      ...
      const tagStartIdx = buffer.indexOf("<familia_");
      if (tagStartIdx === -1) {
      ...
      const fullTagRegex = /<familia_meta>([\s\S]*?)<\/familia_meta>/;
      const match = buffer.match(fullTagRegex);
      if (match) {
        // Extract metadata and remove the tag
        try {
          metadataFound = JSON.parse(match[1]) as FamiliaMetadata;
        } catch {
          // Invalid JSON, ignore
        }
    ...
    getMetadata(): FamiliaMetadata | null {
